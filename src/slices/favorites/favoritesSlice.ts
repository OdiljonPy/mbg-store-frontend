...
export const postFavourites = createAsyncThunk(
	"post_favourites",
	async (ids: number[]) => {
		const request = await API.post("/store/products/feature/", {
			product_ids: ids,
		});
		return request.data;
	}
...
const favoritesSlice = createSlice({
	...
	extraReducers: (builder) => {
			...
			if (payload.ok && payload.result?.length) {
				...
				payload.result?.forEach((product) => {
					...
					if (idx === -1) {
						...
						state.newFavourites = copyProducts?.filter(
							(product) => product.id !== state.favourites[idx].id
						);
					}
				});
				state.total_count = state.favourites?.length;
		...
		builder
			.addCase(updateFavourites.fulfilled, (state, { payload }) => {
				if (payload.ok) {
