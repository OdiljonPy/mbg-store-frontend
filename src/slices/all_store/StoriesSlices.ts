...
import {ICommonStores, ICommonStoreSingle, IStores} from "@/data-types/stores/stores";
...
export const fetchStoreSingle = createAsyncThunk('store_single',async (id:string | string[] | undefined)=>{
...
const storeSlices = createSlice({
    ...
    //     for store single
        builder.addCase(fetchStoreSingle.pending,(state)=>{
            state.loading = true
            state.error = false
        })
