...
import { fetchOftenSearchedProducts } from "@/slices/product/oftenSearchedProductsSlice";
import { fetchSearchKey } from "@/slices/product/productSlices";
import { addSearchHistory } from "@/slices/search_history/searchHistorySlice";
...
const ProductsSearch = (props: props) => {
	...
	useEffect(() => {
		dispatch(fetchOftenSearchedProducts());
	}, [dispatch]);
	...
	const handleWithKey = (e: KeyboardEvent<HTMLInputElement>) => {
	...
	const items: MenuProps["items"] = [
		...
		!searchText.length && oftenSearchedData.length
			? {
					key: 3,
					label: <ProductSearchGroup title={t("search.often")} />,
					type: "group",
					children: [
			...
			>
				<div className={`${css.search} ${focused ? css.focused : ""}`}>
					<div className={css.inputWrapper}>
						...
						/>
						<svg
							className={`${css.icon} ${searchText.length ? css.show : ""}`}
