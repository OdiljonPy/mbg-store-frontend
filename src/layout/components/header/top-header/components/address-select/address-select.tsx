...
const AddressSelect = () => {
	...
	const { address_list, main_address } = useSelector((state: RootState) => state.address);
	...
	const items: MenuProps["items"] = [
			...
			label: (
				<AddLocationModal>
					<button type={"button"} className={`${css.add_btn}`}>
						<svg className={css.add_icon} width='18' height='18' viewBox='0 0 18 18' fill='none' xmlns='http://www.w3.org/2000/svg'>
							<path
		...
		...address_list.map((item) => ({
			label: <AddressItem address_item={item} main_address={main_address.address} />,
		...
		})),
		{
			...
			children: [
				...shippingList.map((item) => ({
					label: <ShippingItem shipping_item={item} main_address={main_address.address} />,
					key: item.latitude + item.longitude,
				})),
			],
		},
	...
	return (
		...
		>
			<Dropdown menu={{ items, style: { borderRadius: 16, maxWidth: 350 } }} trigger={["click"]} className={css.dropdown}>
				<button className={`${css.btn} ${raleway.className}`}>
					<svg className={css.svg} width='18' height='22' viewBox='0 0 18 22' fill='none' xmlns='http://www.w3.org/2000/svg'>
						<path
					...
					<span className={css.text}>{main_address.address ? main_address.address : "Укажите адрес"}</span>
				</button>
			</Dropdown>
		</ConfigProvider>
	);
