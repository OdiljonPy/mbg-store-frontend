...
function ShippingItem({ main_address, shipping_item }: Props) {
	...
	return (
		<div className={css.wrapper}>
			<CustomRadio
				radio={{
					key: String(shipping_item.latitude + shipping_item.longitude),
					name: "address",
					title: "",
				}}
				options={{
					checked: shipping_item.address === main_address,
					disabled: false,
			...
			>
				<div className={css.label}>
					<div className={css.title_wrapper}>
						<h4 className={css.title}>{shipping_item.address_name}</h4>
						{shipping_item.main_address && <Badge className={css.badge}>Основной</Badge>}
					</div>
