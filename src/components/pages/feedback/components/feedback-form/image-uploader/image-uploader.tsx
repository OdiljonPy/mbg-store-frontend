...
const ImageUploader = ({onUploadImage, isHide}: props) => {
    ...
    const {getRootProps, getInputProps} = useDropzone({onDrop: onDrop, multiple: true})
    return (

        <div className={`${css.dropzone} ${isHide ? css.hide : ''}`} {...getRootProps()}>
            <input className={css.input} {...getInputProps()} type={'file'} accept="image/*"/>
            <svg className={css.icon} width="42" height="42" viewBox="0 0 42 42" fill="none"
