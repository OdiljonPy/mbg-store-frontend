...
const DetailCart = (props: props) => {
  ...
  return (
    <div className={css.total}>
      <h3 className={css.title}>{t("order_placed.order_detail")}</h3>
      <div className={css.info}>
        <DetailItem
          className={css.finalPrice}
          label={t("order_placed.number_order")}
          value={last_order?.id}
          loading={loading}
        />
        <DetailItem
          className={css.paddingTop}
          label={t("order_placed.date_order")}
          value={dayjs(last_order.created_at).format("D MMMM YYYY г. H:mm")}
          loading={loading}
        />
        <DetailItem
          className={css.paddingTop}
          label={t("order_placed.recipient")}
          value={last_order?.full_name}
          loading={loading}
        />
        <DetailItem
          className={css.paddingTop}
          label={t("cart.delivery.phone_label")}
          loading={loading}
          value={formatPhoneNumber(
            last_order?.phone_number
              ? last_order.phone_number
              : "+998997777777",
          )}
        />
