...
const ObtainingDelivery = ({
  ...
  const { shippingList, loading } = useSelector(
  ...
  const saveAddress = () => {
    if (activeAddressCart) {
      ...
      console.log(activeAddressCart, "active address cart");
      dispatch(
        patchShipping({
          body: {
            main_address: true,
            latitude: activeAddressCart.latitude,
            longitude: activeAddressCart.longitude,
            address: activeAddressCart.address,
            address_name: activeAddressCart.address_name,
          },
          shippingId: activeAddressCart.id,
        }),
      );
    } else {
  ...
  return (
    <div ref={cardRef}>
      ...
      <div className={css.action_btn}>
        ...
        {shippingList.length ? (
          <SendButton title={"approve"} onClick={() => saveAddress()} />
        ) : (
          ""
        )}
      </div>
