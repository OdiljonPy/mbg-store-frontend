        ...
        <div className={css.obtaining_btn}>
            ...
            <div className={css.choose_btn}>
                <div onClick={(state) => changeTab('left')}>
                 <TruckSVG color={`${tab == 'left'? '#39B969' :'#A4A4A4' }`}/> <span className={`${css.btn_text} ${raleway.className} ${tab == 'left' ? css.active : ''}`}>Доставка</span>
                </div>
                <div onClick={(state)=> changeTab('right')}>
                    <StoreSVG color={`${tab == 'right'? '#39B969' :'#A4A4A4' }`}/> <span className={`${css.btn_text} ${raleway.className} ${tab == 'right' ? css.active : ''}`}>Самовывоз</span>
                </div>
            </div>
        </div>
    )
