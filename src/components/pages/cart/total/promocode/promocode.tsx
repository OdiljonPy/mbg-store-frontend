...
const PromoCode = (props: props) => {
    ...
    const {error,loading} = useSelector((state:RootState)=> state.promo_code)
    ...
    return (
        <form onSubmit={handleSubmit(onAcceptPromo)} className={css.promocode}>
            <div className={css.inputWrapper}>
                <input value={code} onChange={handleInput}  placeholder={t('cart.promo_code')} className={`${inputCss.input} ${err && code ? inputCss.error :''}`} />

                {(err && code) && <FormError error={(t('cart.invalid_code'))} style={{margin:'2px',position:"absolute",fontSize:'14px'}}></FormError>}
            </div>
            <Button type={"submit"} full disabled={!code} loading={loading}>{!loading && t('cart.accept')}</Button>
        </form>
    );
};
