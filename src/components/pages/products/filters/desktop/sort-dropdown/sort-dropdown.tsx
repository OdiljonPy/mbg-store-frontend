...
const SortDropdown = (props: props) => {
	...
	const sort: string | null = searchParams.get("sort") || "popular";
	...
	const items: MenuProps["items"] = sortingOptions.map(({ title, val }) => {
		return {
			label: (
				<CustomRadio
					radio={{ title: t(title), key: val, name: "sort" }}
					options={{
						onChange,
						checked: val === sort,
						disabled: false,
	...
	return (
		...
		>
			<button
				className={`${css.btn} ${!isReady ? css.hide : ""} ${
					raleway.className
				}`}
			>
				<span>{t("fromBeginning")}:</span>{" "}
				<span className={css.sort}>{t(`filters.sorting.${sort}`)}</span>{" "}
				<svg
					className={`${css.icon} ${open ? css.opened : ""}`}
					width='24'
					height='24'
					viewBox='0 0 30 30'
					fill='none'
