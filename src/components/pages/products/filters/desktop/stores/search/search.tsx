...
import {useStoriesSearch} from "@/components/pages/products/filters/desktop/stores/hook/use-stores-search";
...
const Search = (props: props) => {
    ...
    const {searchValue,onSearchValueChange} = useStoriesSearch()

    return (
        <div className={`${css.search} ${active ? css.active : ''}`}>
            <svg className={css.icon} width="21" height="20" viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
            ...
            <input placeholder={t('search.byName')}  onBlur={()=> setActive(false)} onFocus={()=> setActive(true)} value={searchValue} onChange={onSearchValueChange}  className={css.input}/>
        </div>
    );
};
