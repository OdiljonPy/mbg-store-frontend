		...
		return (
			<div className={css.wrapper}>
				{Array.from({ length: 6 }).map((_, index) => (
					<Skeleton
						key={index}
						containerClassName={css[`item${index + 1}`]}
						className={css.skeleton}
					/>
				))}
				<Skeleton containerClassName={css.link_skeleton} height={100} />
			</div>
		);
	}
		...
		<div className={css.wrapper}>
			{categories?.slice(0, 6)?.map((category, index) => (
				<CategoryItem
					category={category}
					key={category.id}
					classNames={css[`item${index + 1}`]}
				/>
			))}
			<Link
				href={"/catalog"}
				className={`${css.link} ${raleway.className}`}
			>
				{t("header.catalog")}
				<svg
					width='28'
